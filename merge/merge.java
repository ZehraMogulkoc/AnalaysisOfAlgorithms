...
public class merge {
	public static void main(String[] args) {
		...
		// Running time of merge sort 19918800
		System.out.printf("Elapsed time in nanoseconds for original merge sort: %d\n", elapsed_time);

		// part (d)
		start_time = System.nanoTime();
		merge_sort_improved(array_2, 0, array_2.length - 1, level, max_level2);
		end_time = System.nanoTime();
		elapsed_time = end_time - start_time;
		...
		for (int max_level = 1; max_level <= 17; max_level++) {
			...
			start_time = System.nanoTime();
			merge_sort_improved(array, 0, array.length - 1, level2, max_level);
			end_time = System.nanoTime();
			elapsed_time = end_time - start_time;
