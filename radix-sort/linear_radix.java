...
public class linear_radix {
	public static void main(String[] args) {
		...
		for (int i = 10; i <= 100000000; i = i * 10) {
			...
			int[] array_3 = new int[i];
			int[] array_4 = new int[i];
			B = new int[i];
			for (int a = 0; a < i; a++) {
				array[a] = rand.nextInt(100,999);
				array_2[a] = array[a];
				array_3[a] = array[a];
				array_4[a] = array[a];
			}
			start_time = System.nanoTime();
			radix_sort(array_4, i, 3);
			end_time = System.nanoTime();
			elapsed_time = end_time - start_time;
			System.out.printf("Elapsed time in nanoseconds for radix sort: %d\n", elapsed_time);

			start_time = System.nanoTime();
			counting_sort2(array, B, 100000000); // couldnt find the size bug of counting sort, so put 100000000 for the code to work 
			end_time = System.nanoTime();
			elapsed_time = end_time - start_time;
		...
		int[] array_4 = new int[100000000];
		int[] B = new int[100000000];
		for (int j = 0; j < 100000000; j++) {
			array[j] = rand.nextInt(100,999);
			array_2[j] = array[j];
			array_3[j] = array[j];
			array_4[j] = array[j];
		}
		
		//radix_sort(array_4, 100000000, 3);
			//counting_sort2(array, B, 10000000);
	//	merge_sort(array_2,0,100000000-1);
	...
	public static void counting_sort2(int[] A, int[] B, int k) { // old radix sort
		int[] C = new int[k];
		for (int i = 0; i < k; i++) {
			C[i] = 0;
		}
	...
	public static void counting_sort(int A[], int n, int div) { // new counting sort
